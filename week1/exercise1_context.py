...
# Dual-provider: Yandex Cloud (primary) > Nebius Token Factory (fallback)
_yandex_key = os.getenv("YANDEX_CLOUD_API_KEY", "").strip()
_yandex_url = os.getenv("YANDEX_BASE_URL", "").strip()
_yandex_folder = os.getenv("YANDEX_CLOUD_FOLDER", "").strip()

if _yandex_key and _yandex_url and _yandex_folder:
    client = OpenAI(api_key=_yandex_key, base_url=_yandex_url)
    _PROVIDER = "yandex"
else:
    client = OpenAI(
...
if _PROVIDER == "yandex":
    MAIN_MODEL  = f"gpt://{_yandex_folder}/yandexgpt/rc"
    SMALL_MODEL = f"gpt://{_yandex_folder}/yandexgpt-lite/latest"
else:
